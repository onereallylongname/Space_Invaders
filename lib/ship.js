function Ship() {
  ...
  this.shoot = function(shoot = false) {
    if(shoot) {
  ...
  this.resetShoots = function () {
    if(this.cooldownCount < this.cooldown){
     this.cooldownCount++;
     console.log(this.cooldownCount);
    }
  }
  ...
  this.cahngeExtrass = function(val){
    this.extrass = clamp(this.extrass + val, 3, 12);
  }
  this.cahngeSSize = function(val){
    this.sSize = clamp(this.sSize + val, 1, 20);
  }
}
